...
import { ShieldCheck, Lock, CreditCard, ChevronRight, Check, Star, Users, Clock, User, Phone, Mail } from 'lucide-react';
...
const CheckoutScreen: React.FC = () => {
  ...
  const handlePayment = () => {
    ...
    // Simulação de processamento seguro antes do redirecionamento
    setTimeout(() => {
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#0a0a0a] text-white font-sans relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-green-500/10 via-transparent to-transparent z-0" />
      
      {/* Header Fixo de Segurança */}
      <div className="bg-[#111]/95 border-b border-white/5 py-4 px-6 flex items-center justify-between shrink-0 z-50 backdrop-blur-xl">
        <div className="flex items-center gap-2">
          <ShieldCheck size={18} className="text-[#25D366]" />
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Checkout Seguro</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden relative z-10 scroll-smooth">
        ...
        <div className="bg-[#25D366]/10 border-b border-[#25D366]/20 py-3 px-6 flex items-center justify-center gap-3">
          <Users size={14} className="text-[#25D366]" />
          <span className="text-[10px] font-bold text-[#25D366] uppercase tracking-widest text-center">
            {activeUsers} Operadores ativos neste momento
          </span>
        </div>
        ...
        <div className="p-6 pb-32 space-y-8 max-w-sm mx-auto w-full animate-slide-up">
          ...
          {/* Opção de Pagamento */}
          <div className="space-y-4">
            <div className="bg-[#25D366]/5 border border-[#25D366]/30 rounded-[2rem] p-5 flex items-center justify-between shadow-lg group relative overflow-hidden">
              <div className="flex items-center gap-4 relative z-10">
                <div className="bg-[#25D366] p-2.5 rounded-xl">
                  <CreditCard size={20} className="text-black" />
                </div>
                <div>
                  <span className="text-[13px] font-black text-white uppercase tracking-tighter block leading-none">Pague com PIX</span>
                  <span className="text-[10px] font-bold text-[#25D366] uppercase mt-1 block">Liberação instantânea</span>
                </div>
              </div>
          ...
          <div className="pt-4 space-y-6">
            <button 
              ...
              className={`w-full py-7 rounded-[2.5rem] font-black text-2xl flex items-center justify-center gap-3 transition-all relative overflow-hidden active:scale-95 shadow-[0_20px_60px_rgba(37,211,102,0.4)] ${
                isProcessing ? 'bg-zinc-800 text-zinc-500' : 'bg-[#25D366] text-black hover:brightness-110'
              }`}
            >
              {isProcessing ? (
            ...
            <div className="flex items-center justify-between py-6 border-t border-white/5">
               <div className="flex flex-col items-start">
                 ...
                 <span className="text-[11px] font-black italic">Aprovação Máxima</span>
               </div>
               <div className="h-10 w-px bg-white/5"></div>
               <div className="text-right">
                 <p className="text-[9px] text-zinc-500 uppercase font-black leading-tight tracking-[0.2em]">
                   Processamento Seguro <br /> <span className="text-white">Via Cakto</span>
                 </p>
               </div>
            </div>
            ...
            <div className="text-center space-y-3 opacity-40">
              <p className="text-[8px] uppercase tracking-[0.4em] font-bold">
                Criptografia 256-bit AES Protegida
              </p>
